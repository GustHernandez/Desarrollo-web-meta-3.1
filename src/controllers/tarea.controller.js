...
const obtenerTodas = (req, res) => {
  try {
    ...
    if(formato === "text"){
        const texto = tareas.map(t => `ID: ${t.id}, Título: ${t.titulo}, Completada: ${t.completada}`).join('\n');
        return res.type("text").send(texto);
    }
...
const buscarTareas = (req, res) => {
  ...
  const resultado = tareas.filter(tarea =>
    tarea.titulo.toLowerCase().includes(q.toLowerCase())
  );
